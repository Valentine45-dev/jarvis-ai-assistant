...
def _extract_steps(raw: str, start_at: int) -> list[str]:
    ...
    # single-line fallback: comma-separated or " and then "
    parts = re.split(r",|\band then\b|\bthen\b", tail, flags=re.IGNORECASE)
...
def parse_create_workflow_command(raw_input: str) -> dict[str, Any] | None:
    ...
    marker_match = re.search(
        r"(?:which has|which have|that has|that have|with steps?|with|have)\s*:?",
        stripped,
        flags=re.IGNORECASE,
    )
    start_at = marker_match.end() if marker_match else stripped.find(":") + 1
    if start_at <= 0:
        return None
