...
| `README.md` | Quickstart, feature overview, requirements, setup instructions. It now points readers back to `context.md` as the canonical project map and summarizes advanced browser automation plus the `JARVIS_BROWSER_USE_LLM_PICKER` kill switch. |
...
7. `JARVIS_BROWSER_USE_LLM_PICKER=false` disables the Haiku picker entirely and routes directly to legacy text/selector behavior.
8. Every failure path (empty snapshot, missing Anthropic key, timeout/rate limit/API error, malformed JSON, unknown ref, locator miss) falls back to the legacy text/selector fill/click chain rather than failing the command outright.
...
| `tests/test_review_fixes.py` | unittest | Regression tests for the recent review fixes: dangerous shell confirmation executes the stored callback, `batch_delete` confirmation includes every candidate, `search_files` rejects bad supplied paths without home fallback, no-path `search_files` still searches home, and qualified missing read paths do not resolve to unrelated same-basename files. |
...
- **OCR requires language data, not just the binary.** On Windows, `tesseract --version` can pass while OCR still fails if `eng.traineddata` is missing from `C:\Program Files\Tesseract-OCR\tessdata\`. No code change is needed for that state; fix the local Tesseract install first and verify `tesseract --list-langs` shows `eng`.
...
| Debug OCR failures | First verify `TESSERACT_CMD`/PATH, then run `tesseract --list-langs`. If `eng` is missing, install or restore `eng.traineddata` in `C:\Program Files\Tesseract-OCR\tessdata\`; do not change JARVIS code until the local Tesseract language list is correct. |
