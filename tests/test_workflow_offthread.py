...
# Note: the Option-A narration-buffering/sequencing/fallback tests were removed
# with that mechanism — row safety is now structural (Option B: row-id anchoring),
# covered by test_syslog_render.py's proxy/panel by-id tests. The narration simply
# appends its own row whenever ready; the primary typewriter is bound to its own
# row id, so ordering no longer matters (and TtsEngine's single worker queue keeps
# the audio ordered).
